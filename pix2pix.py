...
parser.add_argument("--mode", required=True, choices=["train", "test", "export", 'gradcam'])
...
Model = collections.namedtuple("Model",
                               "outputs, predict_real,predict_real_layers, predict_fake,predict_fake_layers, discrim_loss, discrim_grads_and_vars, gen_loss_GAN, gen_loss_L1, gen_grads_and_vars, train")
...
def create_model(inputs, targets):
    ...
    with tf.name_scope("real_discriminator"):
        with tf.variable_scope("discriminator"):
            ...
            predict_real, predict_real_layers = create_discriminator(inputs, targets)

    with tf.name_scope("fake_discriminator"):
        with tf.variable_scope("discriminator", reuse=True):
            # 2x [batch, height, width, channels] => [batch, 30, 30, 1]
            predict_fake, predict_fake_layers = create_discriminator(inputs, outputs)
    ...
    return Model(
        ...
        predict_real_layers=predict_real_layers,
        predict_fake=predict_fake,
        predict_fake_layers=predict_fake_layers,
